...
class AudioTranscriber:
    ...
    def handle_responses(self, responses):
        ...
        for response in responses:
            ...
            # Skip processing if we're speaking or processing
            if self.is_speaking or self.is_processing:
                # Clear the audio queue and transcripts
                while not self.audio_queue.empty():
                    try:
                        self.audio_queue.get_nowait()
                    except queue.Empty:
            ...
            # Reset transcripts when switching to FULL_LISTENING
            if self.listening_state == ListeningState.FULL_LISTENING:
                if not hasattr(self, '_last_state') or self._last_state != ListeningState.FULL_LISTENING:
                    self.aggressive_cleanup()  # Use aggressive cleanup instead
                    self._last_state = ListeningState.FULL_LISTENING
    ...
    def aggressive_cleanup(self):
        ...
        if hasattr(self, 'client'):
            try:
                # Create a new streaming config to reset the recognition state
                self.streaming_config = speech.StreamingRecognitionConfig(
                    config=self.config,
                    interim_results=True
                )
